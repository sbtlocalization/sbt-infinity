// Code generated by kaitai-struct-compiler from a .ksy source file. DO NOT EDIT.

package parser

import (
	"github.com/kaitai-io/kaitai_struct_go_runtime/kaitai"
	"bytes"
	"io"
)


/**
 * 
 * @see <a href="https://gibberlings3.github.io/iesdp/file_formats/ie_formats/cre_v1.htm
 * ">Source</a>
 */

type Cre_SpellType int
const (
	Cre_SpellType__Priest Cre_SpellType = 0
	Cre_SpellType__Wizard Cre_SpellType = 1
	Cre_SpellType__Innate Cre_SpellType = 2
)
var values_Cre_SpellType = map[Cre_SpellType]struct{}{0: {}, 1: {}, 2: {}}
func (v Cre_SpellType) isDefined() bool {
	_, ok := values_Cre_SpellType[v]
	return ok
}
type Cre struct {
	Magic []byte
	Version string
	LongNameRef uint32
	ShortNameRef uint32
	Body *Cre_BodyV1
	_io *kaitai.Stream
	_root *Cre
	_parent kaitai.Struct
}
func NewCre() *Cre {
	return &Cre{
	}
}

func (this Cre) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre) Read(io *kaitai.Stream, parent kaitai.Struct, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp1, err := this._io.ReadBytes(int(4))
	if err != nil {
		return err
	}
	tmp1 = tmp1
	this.Magic = tmp1
	if !(bytes.Equal(this.Magic, []uint8{67, 82, 69, 32})) {
		return kaitai.NewValidationNotEqualError([]uint8{67, 82, 69, 32}, this.Magic, this._io, "/seq/0")
	}
	tmp2, err := this._io.ReadBytes(int(4))
	if err != nil {
		return err
	}
	tmp2 = tmp2
	this.Version = string(tmp2)
	tmp3, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.LongNameRef = uint32(tmp3)
	tmp4, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.ShortNameRef = uint32(tmp4)
	switch (this.Version) {
	case "V1.0":
		tmp5 := NewCre_BodyV1()
		err = tmp5.Read(this._io, this, this._root)
		if err != nil {
			return err
		}
		this.Body = tmp5
	case "V1.1":
		tmp6 := NewCre_BodyV1()
		err = tmp6.Read(this._io, this, this._root)
		if err != nil {
			return err
		}
		this.Body = tmp6
	}
	return err
}
type Cre_BodyV1 struct {
	Header *Cre_BodyV1_Header
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre
	_f_effects bool
	effects []kaitai.Struct
	_f_itemSlots bool
	itemSlots *Cre_BodyV1_ItemSlot
	_f_items bool
	items []*Cre_BodyV1_Item
	_f_knownSpells bool
	knownSpells []*Cre_BodyV1_KnownSpell
	_f_memorizedSpells bool
	memorizedSpells []*Cre_BodyV1_MemorizedSpell
	_f_spellMemorizationInfos bool
	spellMemorizationInfos []*Cre_BodyV1_SpellMemorizationInfo
}
func NewCre_BodyV1() *Cre_BodyV1 {
	return &Cre_BodyV1{
	}
}

func (this Cre_BodyV1) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1) Read(io *kaitai.Stream, parent *Cre, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp7 := NewCre_BodyV1_Header()
	err = tmp7.Read(this._io, this, this._root)
	if err != nil {
		return err
	}
	this.Header = tmp7
	return err
}
func (this *Cre_BodyV1) Effects() (v []kaitai.Struct, err error) {
	if (this._f_effects) {
		return this.effects, nil
	}
	this._f_effects = true
	_pos, err := this._io.Pos()
	if err != nil {
		return nil, err
	}
	_, err = this._io.Seek(int64(this.Header.OfsEffects), io.SeekStart)
	if err != nil {
		return nil, err
	}
	for i := 0; i < int(this.Header.NumEffects); i++ {
		_ = i
		switch (this.Header.EffVersion) {
		case Cre_BodyV1_Header_EffVersion__Version1:
			tmp8 := NewEff_BodyV1()
			err = tmp8.Read(this._io, nil, nil)
			if err != nil {
				return nil, err
			}
			this.effects = append(this.effects, tmp8)
		case Cre_BodyV1_Header_EffVersion__Version2:
			tmp9 := NewEff_BodyV2(true)
			err = tmp9.Read(this._io, nil, nil)
			if err != nil {
				return nil, err
			}
			this.effects = append(this.effects, tmp9)
		}
	}
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return nil, err
	}
	return this.effects, nil
}
func (this *Cre_BodyV1) ItemSlots() (v *Cre_BodyV1_ItemSlot, err error) {
	if (this._f_itemSlots) {
		return this.itemSlots, nil
	}
	this._f_itemSlots = true
	_pos, err := this._io.Pos()
	if err != nil {
		return nil, err
	}
	_, err = this._io.Seek(int64(this.Header.OfsItemSlots), io.SeekStart)
	if err != nil {
		return nil, err
	}
	tmp10 := NewCre_BodyV1_ItemSlot()
	err = tmp10.Read(this._io, this, this._root)
	if err != nil {
		return nil, err
	}
	this.itemSlots = tmp10
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return nil, err
	}
	return this.itemSlots, nil
}
func (this *Cre_BodyV1) Items() (v []*Cre_BodyV1_Item, err error) {
	if (this._f_items) {
		return this.items, nil
	}
	this._f_items = true
	_pos, err := this._io.Pos()
	if err != nil {
		return nil, err
	}
	_, err = this._io.Seek(int64(this.Header.OfsItems), io.SeekStart)
	if err != nil {
		return nil, err
	}
	for i := 0; i < int(this.Header.NumItems); i++ {
		_ = i
		tmp11 := NewCre_BodyV1_Item()
		err = tmp11.Read(this._io, this, this._root)
		if err != nil {
			return nil, err
		}
		this.items = append(this.items, tmp11)
	}
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return nil, err
	}
	return this.items, nil
}
func (this *Cre_BodyV1) KnownSpells() (v []*Cre_BodyV1_KnownSpell, err error) {
	if (this._f_knownSpells) {
		return this.knownSpells, nil
	}
	this._f_knownSpells = true
	_pos, err := this._io.Pos()
	if err != nil {
		return nil, err
	}
	_, err = this._io.Seek(int64(this.Header.OfsKnownSpells), io.SeekStart)
	if err != nil {
		return nil, err
	}
	for i := 0; i < int(this.Header.NumKnownSpells); i++ {
		_ = i
		tmp12 := NewCre_BodyV1_KnownSpell()
		err = tmp12.Read(this._io, this, this._root)
		if err != nil {
			return nil, err
		}
		this.knownSpells = append(this.knownSpells, tmp12)
	}
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return nil, err
	}
	return this.knownSpells, nil
}

/**
 * This section details which spells the character has memorized. It consists of an array of entries formatted as follows.
 */
func (this *Cre_BodyV1) MemorizedSpells() (v []*Cre_BodyV1_MemorizedSpell, err error) {
	if (this._f_memorizedSpells) {
		return this.memorizedSpells, nil
	}
	this._f_memorizedSpells = true
	_pos, err := this._io.Pos()
	if err != nil {
		return nil, err
	}
	_, err = this._io.Seek(int64(this.Header.OfsMemorizedSpells), io.SeekStart)
	if err != nil {
		return nil, err
	}
	for i := 0; i < int(this.Header.NumMemorizedSpells); i++ {
		_ = i
		tmp13 := NewCre_BodyV1_MemorizedSpell()
		err = tmp13.Read(this._io, this, this._root)
		if err != nil {
			return nil, err
		}
		this.memorizedSpells = append(this.memorizedSpells, tmp13)
	}
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return nil, err
	}
	return this.memorizedSpells, nil
}

/**
 * This section details how many spells the creature can memorize, and how many it has memorized. It consists of an array of entries.
 */
func (this *Cre_BodyV1) SpellMemorizationInfos() (v []*Cre_BodyV1_SpellMemorizationInfo, err error) {
	if (this._f_spellMemorizationInfos) {
		return this.spellMemorizationInfos, nil
	}
	this._f_spellMemorizationInfos = true
	_pos, err := this._io.Pos()
	if err != nil {
		return nil, err
	}
	_, err = this._io.Seek(int64(this.Header.OfsSpellMemorizationInfo), io.SeekStart)
	if err != nil {
		return nil, err
	}
	for i := 0; i < int(this.Header.NumSpellMemorizationInfo); i++ {
		_ = i
		tmp14 := NewCre_BodyV1_SpellMemorizationInfo()
		err = tmp14.Read(this._io, this, this._root)
		if err != nil {
			return nil, err
		}
		this.spellMemorizationInfos = append(this.spellMemorizationInfos, tmp14)
	}
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return nil, err
	}
	return this.spellMemorizationInfos, nil
}

type Cre_BodyV1_Header_EffVersion int
const (
	Cre_BodyV1_Header_EffVersion__Version1 Cre_BodyV1_Header_EffVersion = 0
	Cre_BodyV1_Header_EffVersion__Version2 Cre_BodyV1_Header_EffVersion = 1
)
var values_Cre_BodyV1_Header_EffVersion = map[Cre_BodyV1_Header_EffVersion]struct{}{0: {}, 1: {}}
func (v Cre_BodyV1_Header_EffVersion) isDefined() bool {
	_, ok := values_Cre_BodyV1_Header_EffVersion[v]
	return ok
}
type Cre_BodyV1_Header struct {
	Flags *Cre_BodyV1_Header_Flags
	XpForKilling uint32
	XpPowerLevel uint32
	GoldCarried uint32
	Status *Cre_BodyV1_Header_StatusFlags
	CurrentHp uint16
	MaximumHp uint16
	AnimationId uint32
	MetalColorIndex uint8
	MinorColorIndex uint8
	MajorColorIndex uint8
	SkinColorIndex uint8
	LeatherColorIndex uint8
	ArmorColorIndex uint8
	HairColorIndex uint8
	EffVersion Cre_BodyV1_Header_EffVersion
	SmallPortrait string
	LargePortrait string
	Reputation int8
	HideInShadows uint8
	NaturalAc int16
	EffectiveAc int16
	CrushingAcModifier int16
	MissileAcModifier int16
	PiercingAcModifier int16
	SlashingAcModifier int16
	Thac0 uint8
	NumAttacks uint8
	SaveVsDeath uint8
	SaveVsWands uint8
	SaveVsPolymorph uint8
	SaveVsBreathAttacks uint8
	SaveVsSpells uint8
	ResistFire uint8
	ResistCold uint8
	ResistElectricity uint8
	ResistAcid uint8
	ResistMagic uint8
	ResistMagicFire uint8
	ResistMagicCold uint8
	ResistSlashing uint8
	ResistCrushing uint8
	ResistPiercing uint8
	ResistMissile uint8
	DetectIllusion uint8
	ProficienciesUnspent uint8
	SetTraps uint8
	Lore uint8
	Lockpicking uint8
	MoveSilently uint8
	FindDisarmTraps uint8
	PickPockets uint8
	Fatigue uint8
	Intoxication uint8
	Luck uint8
	Proficiencies kaitai.Struct
	TurnUndeadLevel uint8
	TrackingSkill uint8
	TrackingTargetOrPsteeFlags []byte
	StrRefs []uint32
	LevelFirstClass uint8
	LevelSecondClass uint8
	LevelThirdClass uint8
	Sex uint8
	Strength uint8
	StrengthBonus uint8
	Intelligence uint8
	Wisdom uint8
	Dexterity uint8
	Constitution uint8
	Charisma uint8
	Morale uint8
	MoraleBreak uint8
	RacialEnemy uint8
	MoraleRecoveryTime uint16
	KitOrDeity *Cre_BodyV1_Header_KitDeityField
	OverrideScript string
	ClassScript string
	RaceScript string
	GeneralScript string
	DefaultScript string
	PstExtension *Cre_BodyV1_Header_PstExtensionBlock
	EnemyAlly uint8
	General uint8
	Race uint8
	Class uint8
	Specific uint8
	Gender uint8
	ObjectIdsRefs []uint8
	Alignment uint8
	GlobalActorEnumValue uint16
	LocalActorEnumValue uint16
	DeathVariable string
	OfsKnownSpells uint32
	NumKnownSpells uint32
	OfsSpellMemorizationInfo uint32
	NumSpellMemorizationInfo uint32
	OfsMemorizedSpells uint32
	NumMemorizedSpells uint32
	OfsItemSlots uint32
	OfsItems uint32
	NumItems uint32
	OfsEffects uint32
	NumEffects uint32
	Dialog string
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1
	_raw_KitOrDeity []byte
}
func NewCre_BodyV1_Header() *Cre_BodyV1_Header {
	return &Cre_BodyV1_Header{
	}
}

func (this Cre_BodyV1_Header) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Header) Read(io *kaitai.Stream, parent *Cre_BodyV1, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp15 := NewCre_BodyV1_Header_Flags()
	err = tmp15.Read(this._io, this, this._root)
	if err != nil {
		return err
	}
	this.Flags = tmp15
	tmp16, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.XpForKilling = uint32(tmp16)
	tmp17, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.XpPowerLevel = uint32(tmp17)
	tmp18, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.GoldCarried = uint32(tmp18)
	tmp19 := NewCre_BodyV1_Header_StatusFlags()
	err = tmp19.Read(this._io, this, this._root)
	if err != nil {
		return err
	}
	this.Status = tmp19
	tmp20, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.CurrentHp = uint16(tmp20)
	tmp21, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.MaximumHp = uint16(tmp21)
	tmp22, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.AnimationId = uint32(tmp22)
	tmp23, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.MetalColorIndex = tmp23
	tmp24, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.MinorColorIndex = tmp24
	tmp25, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.MajorColorIndex = tmp25
	tmp26, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SkinColorIndex = tmp26
	tmp27, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.LeatherColorIndex = tmp27
	tmp28, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ArmorColorIndex = tmp28
	tmp29, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.HairColorIndex = tmp29
	tmp30, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.EffVersion = Cre_BodyV1_Header_EffVersion(tmp30)
	tmp31, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp31 = kaitai.BytesTerminate(tmp31, 0, false)
	this.SmallPortrait = string(tmp31)
	tmp32, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp32 = kaitai.BytesTerminate(tmp32, 0, false)
	this.LargePortrait = string(tmp32)
	tmp33, err := this._io.ReadS1()
	if err != nil {
		return err
	}
	this.Reputation = tmp33
	tmp34, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.HideInShadows = tmp34
	tmp35, err := this._io.ReadS2le()
	if err != nil {
		return err
	}
	this.NaturalAc = int16(tmp35)
	tmp36, err := this._io.ReadS2le()
	if err != nil {
		return err
	}
	this.EffectiveAc = int16(tmp36)
	tmp37, err := this._io.ReadS2le()
	if err != nil {
		return err
	}
	this.CrushingAcModifier = int16(tmp37)
	tmp38, err := this._io.ReadS2le()
	if err != nil {
		return err
	}
	this.MissileAcModifier = int16(tmp38)
	tmp39, err := this._io.ReadS2le()
	if err != nil {
		return err
	}
	this.PiercingAcModifier = int16(tmp39)
	tmp40, err := this._io.ReadS2le()
	if err != nil {
		return err
	}
	this.SlashingAcModifier = int16(tmp40)
	tmp41, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Thac0 = tmp41
	tmp42, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.NumAttacks = tmp42
	tmp43, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SaveVsDeath = tmp43
	tmp44, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SaveVsWands = tmp44
	tmp45, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SaveVsPolymorph = tmp45
	tmp46, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SaveVsBreathAttacks = tmp46
	tmp47, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SaveVsSpells = tmp47
	tmp48, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistFire = tmp48
	tmp49, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistCold = tmp49
	tmp50, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistElectricity = tmp50
	tmp51, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistAcid = tmp51
	tmp52, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistMagic = tmp52
	tmp53, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistMagicFire = tmp53
	tmp54, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistMagicCold = tmp54
	tmp55, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistSlashing = tmp55
	tmp56, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistCrushing = tmp56
	tmp57, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistPiercing = tmp57
	tmp58, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ResistMissile = tmp58
	if (this._root.Version == "V1.0") {
		tmp59, err := this._io.ReadU1()
		if err != nil {
			return err
		}
		this.DetectIllusion = tmp59
	}
	if (this._root.Version == "V1.1") {
		tmp60, err := this._io.ReadU1()
		if err != nil {
			return err
		}
		this.ProficienciesUnspent = tmp60
	}
	tmp61, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SetTraps = tmp61
	tmp62, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Lore = tmp62
	tmp63, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Lockpicking = tmp63
	tmp64, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.MoveSilently = tmp64
	tmp65, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.FindDisarmTraps = tmp65
	tmp66, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.PickPockets = tmp66
	tmp67, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Fatigue = tmp67
	tmp68, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Intoxication = tmp68
	tmp69, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Luck = tmp69
	switch (this._root.Version) {
	case "V1.0":
		tmp70 := NewCre_BodyV1_Header_ProficienciesV10()
		err = tmp70.Read(this._io, this, this._root)
		if err != nil {
			return err
		}
		this.Proficiencies = tmp70
	case "V1.1":
		tmp71 := NewCre_BodyV1_Header_ProficienciesV11()
		err = tmp71.Read(this._io, this, this._root)
		if err != nil {
			return err
		}
		this.Proficiencies = tmp71
	}
	tmp72, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.TurnUndeadLevel = tmp72
	tmp73, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.TrackingSkill = tmp73
	tmp74, err := this._io.ReadBytes(int(32))
	if err != nil {
		return err
	}
	tmp74 = tmp74
	this.TrackingTargetOrPsteeFlags = tmp74
	for i := 0; i < int(100); i++ {
		_ = i
		tmp75, err := this._io.ReadU4le()
		if err != nil {
			return err
		}
		this.StrRefs = append(this.StrRefs, tmp75)
	}
	tmp76, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.LevelFirstClass = tmp76
	tmp77, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.LevelSecondClass = tmp77
	tmp78, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.LevelThirdClass = tmp78
	tmp79, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Sex = tmp79
	tmp80, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Strength = tmp80
	tmp81, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.StrengthBonus = tmp81
	tmp82, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Intelligence = tmp82
	tmp83, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Wisdom = tmp83
	tmp84, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Dexterity = tmp84
	tmp85, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Constitution = tmp85
	tmp86, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Charisma = tmp86
	tmp87, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Morale = tmp87
	tmp88, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.MoraleBreak = tmp88
	tmp89, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.RacialEnemy = tmp89
	tmp90, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.MoraleRecoveryTime = uint16(tmp90)
	tmp91, err := this._io.ReadBytes(int(4))
	if err != nil {
		return err
	}
	tmp91 = tmp91
	this._raw_KitOrDeity = tmp91
	_io__raw_KitOrDeity := kaitai.NewStream(bytes.NewReader(this._raw_KitOrDeity))
	tmp92 := NewCre_BodyV1_Header_KitDeityField()
	err = tmp92.Read(_io__raw_KitOrDeity, this, this._root)
	if err != nil {
		return err
	}
	this.KitOrDeity = tmp92
	tmp93, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp93 = kaitai.BytesTerminate(tmp93, 0, false)
	this.OverrideScript = string(tmp93)
	tmp94, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp94 = kaitai.BytesTerminate(tmp94, 0, false)
	this.ClassScript = string(tmp94)
	tmp95, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp95 = kaitai.BytesTerminate(tmp95, 0, false)
	this.RaceScript = string(tmp95)
	tmp96, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp96 = kaitai.BytesTerminate(tmp96, 0, false)
	this.GeneralScript = string(tmp96)
	tmp97, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp97 = kaitai.BytesTerminate(tmp97, 0, false)
	this.DefaultScript = string(tmp97)
	if (this._root.Version == "V1.1") {
		tmp98 := NewCre_BodyV1_Header_PstExtensionBlock()
		err = tmp98.Read(this._io, this, this._root)
		if err != nil {
			return err
		}
		this.PstExtension = tmp98
	}
	tmp99, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.EnemyAlly = tmp99
	tmp100, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.General = tmp100
	tmp101, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Race = tmp101
	tmp102, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Class = tmp102
	tmp103, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Specific = tmp103
	tmp104, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Gender = tmp104
	for i := 0; i < int(5); i++ {
		_ = i
		tmp105, err := this._io.ReadU1()
		if err != nil {
			return err
		}
		this.ObjectIdsRefs = append(this.ObjectIdsRefs, tmp105)
	}
	tmp106, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Alignment = tmp106
	tmp107, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.GlobalActorEnumValue = uint16(tmp107)
	tmp108, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.LocalActorEnumValue = uint16(tmp108)
	tmp109, err := this._io.ReadBytes(int(32))
	if err != nil {
		return err
	}
	tmp109 = kaitai.BytesTerminate(tmp109, 0, false)
	this.DeathVariable = string(tmp109)
	tmp110, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OfsKnownSpells = uint32(tmp110)
	tmp111, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.NumKnownSpells = uint32(tmp111)
	tmp112, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OfsSpellMemorizationInfo = uint32(tmp112)
	tmp113, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.NumSpellMemorizationInfo = uint32(tmp113)
	tmp114, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OfsMemorizedSpells = uint32(tmp114)
	tmp115, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.NumMemorizedSpells = uint32(tmp115)
	tmp116, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OfsItemSlots = uint32(tmp116)
	tmp117, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OfsItems = uint32(tmp117)
	tmp118, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.NumItems = uint32(tmp118)
	tmp119, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OfsEffects = uint32(tmp119)
	tmp120, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.NumEffects = uint32(tmp120)
	tmp121, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp121 = kaitai.BytesTerminate(tmp121, 0, false)
	this.Dialog = string(tmp121)
	return err
}

/**
 * PSTEE: BAM
 * Other games: BMP
 */

/**
 * Number of attacks per round
 */

/**
 * Minimum value – 0
 */

/**
 * Lore is calculated as ((level * rate) + int_bonus + wis_bonus). Intelligence and wisdom
 * bonuses are from LOREBON.2DA and the rate is the lookup value in LORE.2DA, based on class.
 * For multiclass characters, (level * rate) is calculated for both classes separately and
 * the higher of the two values is used - they are not cumulative.
 */

/**
 * Minimum value – 0
 */

/**
 * Minimum value – 0
 */

/**
 * Minimum value – 0
 */

/**
 * Minimum value – 0
 */

/**
 * Sex (`GENDER.IDS`) - checkable via the SEX stat.
 * 
 * EE only: determines casting sound prefix.
 * Known values include:
 * - 1 (Male), default – `CHA_M*.WAV`
 * - 2 (Female) – `CHA_F*.WAV`
 * - 3 (Other), 4 (Niether) – `CHA_S*.WAV`
 */

/**
 * 0..100%
 */

/**
 * default – 10
 */

/**
 * See `RACE.IDS`
 */

/**
 * See `EA.IDS`
 */

/**
 * See `GENERAL.IDS`
 */

/**
 * See `RACE.IDS`
 */

/**
 * See `CLASS.IDS`
 */

/**
 * See `SPECIFIC.IDS`
 */

/**
 * See `GENDER.IDS`
 */

/**
 * See `OBJECT.IDS`
 */

/**
 * See `ALIGNMEN.IDS`
 */
type Cre_BodyV1_Header_Flags struct {
	ShowLongnameInTooltip bool
	NoCorpse bool
	KeepCorpse bool
	OriginalClassFighter bool
	OriginalClassMage bool
	OriginalClassCleric bool
	OriginalClassThief bool
	OriginalClassDruid bool
	OriginalClassRanger bool
	FallenPaladin bool
	FallenRanger bool
	Exportable bool
	HideInjuryStatus bool
	AffectedByAlternativeDamage bool
	MovingBetweenAreas bool
	BeenInParty bool
	RestoreItemInHand bool
	ResetRestoringItemInHand bool
	Reserved1 bool
	Reserved2 bool
	PreventExplodingDeath bool
	Reserved3 bool
	IgnoreNightmareModifiers bool
	NoTooltip bool
	AllegianceTracking bool
	GeneralTracking bool
	RaceTracking bool
	ClassTracking bool
	SpecificTracking bool
	GenderTracking bool
	AlignmentTracking bool
	Uninterruptable bool
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_Header
}
func NewCre_BodyV1_Header_Flags() *Cre_BodyV1_Header_Flags {
	return &Cre_BodyV1_Header_Flags{
	}
}

func (this Cre_BodyV1_Header_Flags) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Header_Flags) Read(io *kaitai.Stream, parent *Cre_BodyV1_Header, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp122, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.ShowLongnameInTooltip = tmp122 != 0
	tmp123, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.NoCorpse = tmp123 != 0
	tmp124, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.KeepCorpse = tmp124 != 0
	tmp125, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.OriginalClassFighter = tmp125 != 0
	tmp126, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.OriginalClassMage = tmp126 != 0
	tmp127, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.OriginalClassCleric = tmp127 != 0
	tmp128, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.OriginalClassThief = tmp128 != 0
	tmp129, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.OriginalClassDruid = tmp129 != 0
	tmp130, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.OriginalClassRanger = tmp130 != 0
	tmp131, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.FallenPaladin = tmp131 != 0
	tmp132, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.FallenRanger = tmp132 != 0
	tmp133, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Exportable = tmp133 != 0
	tmp134, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.HideInjuryStatus = tmp134 != 0
	tmp135, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.AffectedByAlternativeDamage = tmp135 != 0
	tmp136, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.MovingBetweenAreas = tmp136 != 0
	tmp137, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.BeenInParty = tmp137 != 0
	tmp138, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.RestoreItemInHand = tmp138 != 0
	tmp139, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.ResetRestoringItemInHand = tmp139 != 0
	tmp140, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Reserved1 = tmp140 != 0
	tmp141, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Reserved2 = tmp141 != 0
	tmp142, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.PreventExplodingDeath = tmp142 != 0
	tmp143, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Reserved3 = tmp143 != 0
	tmp144, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.IgnoreNightmareModifiers = tmp144 != 0
	tmp145, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.NoTooltip = tmp145 != 0
	tmp146, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.AllegianceTracking = tmp146 != 0
	tmp147, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.GeneralTracking = tmp147 != 0
	tmp148, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.RaceTracking = tmp148 != 0
	tmp149, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.ClassTracking = tmp149 != 0
	tmp150, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.SpecificTracking = tmp150 != 0
	tmp151, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.GenderTracking = tmp151 != 0
	tmp152, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.AlignmentTracking = tmp152 != 0
	tmp153, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Uninterruptable = tmp153 != 0
	return err
}

/**
 * In BG1/BG2 this is a 4-byte kit value (KIT.IDS).
 * In PST/PSTEE this is split into deity (DEITY.IDS) and mage type (MAGESPEC.IDS).
 */
type Cre_BodyV1_Header_KitDeityField struct {
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_Header
	_f_bgKit bool
	bgKit uint32
	_f_pstDeity bool
	pstDeity uint16
	_f_pstMageType bool
	pstMageType uint16
}
func NewCre_BodyV1_Header_KitDeityField() *Cre_BodyV1_Header_KitDeityField {
	return &Cre_BodyV1_Header_KitDeityField{
	}
}

func (this Cre_BodyV1_Header_KitDeityField) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Header_KitDeityField) Read(io *kaitai.Stream, parent *Cre_BodyV1_Header, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	return err
}

/**
 * See KIT.IDS
 */
func (this *Cre_BodyV1_Header_KitDeityField) BgKit() (v uint32, err error) {
	if (this._f_bgKit) {
		return this.bgKit, nil
	}
	this._f_bgKit = true
	_pos, err := this._io.Pos()
	if err != nil {
		return 0, err
	}
	_, err = this._io.Seek(int64(0), io.SeekStart)
	if err != nil {
		return 0, err
	}
	tmp154, err := this._io.ReadU4le()
	if err != nil {
		return 0, err
	}
	this.bgKit = tmp154
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return 0, err
	}
	return this.bgKit, nil
}

/**
 * See DEITY.IDS
 */
func (this *Cre_BodyV1_Header_KitDeityField) PstDeity() (v uint16, err error) {
	if (this._f_pstDeity) {
		return this.pstDeity, nil
	}
	this._f_pstDeity = true
	_pos, err := this._io.Pos()
	if err != nil {
		return 0, err
	}
	_, err = this._io.Seek(int64(0), io.SeekStart)
	if err != nil {
		return 0, err
	}
	tmp155, err := this._io.ReadU2le()
	if err != nil {
		return 0, err
	}
	this.pstDeity = tmp155
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return 0, err
	}
	return this.pstDeity, nil
}

/**
 * See MAGESPEC.IDS
 */
func (this *Cre_BodyV1_Header_KitDeityField) PstMageType() (v uint16, err error) {
	if (this._f_pstMageType) {
		return this.pstMageType, nil
	}
	this._f_pstMageType = true
	_pos, err := this._io.Pos()
	if err != nil {
		return 0, err
	}
	_, err = this._io.Seek(int64(2), io.SeekStart)
	if err != nil {
		return 0, err
	}
	tmp156, err := this._io.ReadU2le()
	if err != nil {
		return 0, err
	}
	this.pstMageType = tmp156
	_, err = this._io.Seek(_pos, io.SeekStart)
	if err != nil {
		return 0, err
	}
	return this.pstMageType, nil
}
type Cre_BodyV1_Header_ProficienciesV10 struct {
	LargeSwordsProficiency uint8
	SmallSwordsProficiency uint8
	BowsProficiency uint8
	SpearsProficiency uint8
	BluntProficiency uint8
	SpikedProficiency uint8
	AxeProficiency uint8
	MissileProficiency uint8
	ReservedProficiency []uint8
	UnspentProficiencies uint8
	NumAvailableInventorySlots uint8
	NightmareModeModifiers uint8
	Translucency uint8
	ReputationGainLossWhenKilledOrMurderVariableIncrement uint8
	ReputationGainLossWhenJoiningParty uint8
	ReputationGainLossWhenLeavingParty uint8
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_Header
}
func NewCre_BodyV1_Header_ProficienciesV10() *Cre_BodyV1_Header_ProficienciesV10 {
	return &Cre_BodyV1_Header_ProficienciesV10{
	}
}

func (this Cre_BodyV1_Header_ProficienciesV10) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Header_ProficienciesV10) Read(io *kaitai.Stream, parent *Cre_BodyV1_Header, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp157, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.LargeSwordsProficiency = tmp157
	tmp158, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SmallSwordsProficiency = tmp158
	tmp159, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.BowsProficiency = tmp159
	tmp160, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SpearsProficiency = tmp160
	tmp161, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.BluntProficiency = tmp161
	tmp162, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.SpikedProficiency = tmp162
	tmp163, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.AxeProficiency = tmp163
	tmp164, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.MissileProficiency = tmp164
	for i := 0; i < int(5); i++ {
		_ = i
		tmp165, err := this._io.ReadU1()
		if err != nil {
			return err
		}
		this.ReservedProficiency = append(this.ReservedProficiency, tmp165)
	}
	tmp166, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.UnspentProficiencies = tmp166
	tmp167, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.NumAvailableInventorySlots = tmp167
	tmp168, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.NightmareModeModifiers = tmp168
	tmp169, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Translucency = tmp169
	tmp170, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ReputationGainLossWhenKilledOrMurderVariableIncrement = tmp170
	tmp171, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ReputationGainLossWhenJoiningParty = tmp171
	tmp172, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ReputationGainLossWhenLeavingParty = tmp172
	return err
}
type Cre_BodyV1_Header_ProficienciesV11 struct {
	FistProficiency uint8
	EdgedWeaponProficiency uint8
	HammerProficiency uint8
	AxeProficiency uint8
	ClubProficiency uint8
	BowProficiency uint8
	Reserved []byte
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_Header
}
func NewCre_BodyV1_Header_ProficienciesV11() *Cre_BodyV1_Header_ProficienciesV11 {
	return &Cre_BodyV1_Header_ProficienciesV11{
	}
}

func (this Cre_BodyV1_Header_ProficienciesV11) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Header_ProficienciesV11) Read(io *kaitai.Stream, parent *Cre_BodyV1_Header, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp173, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.FistProficiency = tmp173
	tmp174, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.EdgedWeaponProficiency = tmp174
	tmp175, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.HammerProficiency = tmp175
	tmp176, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.AxeProficiency = tmp176
	tmp177, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.ClubProficiency = tmp177
	tmp178, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.BowProficiency = tmp178
	tmp179, err := this._io.ReadBytes(int(14))
	if err != nil {
		return err
	}
	tmp179 = tmp179
	this.Reserved = tmp179
	return err
}

/**
 * PST-specific extended section. 164 bytes.
 * Only present in V1.1 (and V1.2) CRE files.
 */
type Cre_BodyV1_Header_PstExtensionBlock struct {
	Reserved1 []byte
	Reserved2 []byte
	Reserved3 []byte
	OfsOverlays uint32
	OverlaysSize uint32
	XpSecondClass uint32
	XpThirdClass uint32
	InternalValues []uint16
	GoodIncrement uint8
	LawIncrement uint8
	LadyIncrement uint8
	MurderIncrement uint8
	CharacterType string
	DialogActivationRadius uint8
	CollisionRadius uint8
	Reserved4 []byte
	NumColors uint8
	Attributes uint32
	ColorValues []uint16
	Reserved5 []byte
	ColorPlacements []uint8
	Reserved6 []byte
	Species uint8
	Team uint8
	Faction uint8
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_Header
}
func NewCre_BodyV1_Header_PstExtensionBlock() *Cre_BodyV1_Header_PstExtensionBlock {
	return &Cre_BodyV1_Header_PstExtensionBlock{
	}
}

func (this Cre_BodyV1_Header_PstExtensionBlock) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Header_PstExtensionBlock) Read(io *kaitai.Stream, parent *Cre_BodyV1_Header, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp180, err := this._io.ReadBytes(int(24))
	if err != nil {
		return err
	}
	tmp180 = tmp180
	this.Reserved1 = tmp180
	tmp181, err := this._io.ReadBytes(int(4))
	if err != nil {
		return err
	}
	tmp181 = tmp181
	this.Reserved2 = tmp181
	tmp182, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp182 = tmp182
	this.Reserved3 = tmp182
	tmp183, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OfsOverlays = uint32(tmp183)
	tmp184, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.OverlaysSize = uint32(tmp184)
	tmp185, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.XpSecondClass = uint32(tmp185)
	tmp186, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.XpThirdClass = uint32(tmp186)
	for i := 0; i < int(10); i++ {
		_ = i
		tmp187, err := this._io.ReadU2le()
		if err != nil {
			return err
		}
		this.InternalValues = append(this.InternalValues, tmp187)
	}
	tmp188, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.GoodIncrement = tmp188
	tmp189, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.LawIncrement = tmp189
	tmp190, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.LadyIncrement = tmp190
	tmp191, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.MurderIncrement = tmp191
	tmp192, err := this._io.ReadBytes(int(32))
	if err != nil {
		return err
	}
	tmp192 = kaitai.BytesTerminate(tmp192, 0, false)
	this.CharacterType = string(tmp192)
	tmp193, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.DialogActivationRadius = tmp193
	tmp194, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.CollisionRadius = tmp194
	tmp195, err := this._io.ReadBytes(int(1))
	if err != nil {
		return err
	}
	tmp195 = tmp195
	this.Reserved4 = tmp195
	tmp196, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.NumColors = tmp196
	tmp197, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.Attributes = uint32(tmp197)
	for i := 0; i < int(7); i++ {
		_ = i
		tmp198, err := this._io.ReadU2le()
		if err != nil {
			return err
		}
		this.ColorValues = append(this.ColorValues, tmp198)
	}
	tmp199, err := this._io.ReadBytes(int(3))
	if err != nil {
		return err
	}
	tmp199 = tmp199
	this.Reserved5 = tmp199
	for i := 0; i < int(7); i++ {
		_ = i
		tmp200, err := this._io.ReadU1()
		if err != nil {
			return err
		}
		this.ColorPlacements = append(this.ColorPlacements, tmp200)
	}
	tmp201, err := this._io.ReadBytes(int(21))
	if err != nil {
		return err
	}
	tmp201 = tmp201
	this.Reserved6 = tmp201
	tmp202, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Species = tmp202
	tmp203, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Team = tmp203
	tmp204, err := this._io.ReadU1()
	if err != nil {
		return err
	}
	this.Faction = tmp204
	return err
}

/**
 * See INTERNAL.IDS
 */

/**
 * PST attribute flags
 */

/**
 * See RACE.IDS
 */

/**
 * See TEAM.IDS
 */

/**
 * See FACTION.IDS
 */
type Cre_BodyV1_Header_StatusFlags struct {
	Sleeping bool
	Berserk bool
	Panic bool
	Stunned bool
	Invisible bool
	Helpless bool
	FrozenDeath bool
	StoneDeath bool
	ExplodingDeath bool
	FlameDeath bool
	AcidDeath bool
	Dead bool
	Silenced bool
	Charmed bool
	Poisoned bool
	Hasted bool
	Slowed bool
	Infravision bool
	Blind bool
	DiseasedOrDeactivated bool
	FeebleMinded bool
	NonDetection bool
	ImprovedVisibility bool
	Bless bool
	Chant bool
	DrawUponHolyMight bool
	Luck bool
	Aid bool
	Chantbad bool
	Blur bool
	MirrorImage bool
	Confused bool
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_Header
}
func NewCre_BodyV1_Header_StatusFlags() *Cre_BodyV1_Header_StatusFlags {
	return &Cre_BodyV1_Header_StatusFlags{
	}
}

func (this Cre_BodyV1_Header_StatusFlags) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Header_StatusFlags) Read(io *kaitai.Stream, parent *Cre_BodyV1_Header, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp205, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Sleeping = tmp205 != 0
	tmp206, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Berserk = tmp206 != 0
	tmp207, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Panic = tmp207 != 0
	tmp208, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Stunned = tmp208 != 0
	tmp209, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Invisible = tmp209 != 0
	tmp210, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Helpless = tmp210 != 0
	tmp211, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.FrozenDeath = tmp211 != 0
	tmp212, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.StoneDeath = tmp212 != 0
	tmp213, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.ExplodingDeath = tmp213 != 0
	tmp214, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.FlameDeath = tmp214 != 0
	tmp215, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.AcidDeath = tmp215 != 0
	tmp216, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Dead = tmp216 != 0
	tmp217, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Silenced = tmp217 != 0
	tmp218, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Charmed = tmp218 != 0
	tmp219, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Poisoned = tmp219 != 0
	tmp220, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Hasted = tmp220 != 0
	tmp221, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Slowed = tmp221 != 0
	tmp222, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Infravision = tmp222 != 0
	tmp223, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Blind = tmp223 != 0
	tmp224, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.DiseasedOrDeactivated = tmp224 != 0
	tmp225, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.FeebleMinded = tmp225 != 0
	tmp226, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.NonDetection = tmp226 != 0
	tmp227, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.ImprovedVisibility = tmp227 != 0
	tmp228, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Bless = tmp228 != 0
	tmp229, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Chant = tmp229 != 0
	tmp230, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.DrawUponHolyMight = tmp230 != 0
	tmp231, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Luck = tmp231 != 0
	tmp232, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Aid = tmp232 != 0
	tmp233, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Chantbad = tmp233 != 0
	tmp234, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Blur = tmp234 != 0
	tmp235, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.MirrorImage = tmp235 != 0
	tmp236, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Confused = tmp236 != 0
	return err
}
type Cre_BodyV1_Item struct {
	ResName string
	Duration uint16
	QuantityCharges []uint16
	Flags *Cre_BodyV1_Item_ItemFlags
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1
	_raw_Flags []byte
}
func NewCre_BodyV1_Item() *Cre_BodyV1_Item {
	return &Cre_BodyV1_Item{
	}
}

func (this Cre_BodyV1_Item) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Item) Read(io *kaitai.Stream, parent *Cre_BodyV1, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp237, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp237 = kaitai.BytesTerminate(tmp237, 0, false)
	this.ResName = string(tmp237)
	tmp238, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.Duration = uint16(tmp238)
	for i := 0; i < int(3); i++ {
		_ = i
		tmp239, err := this._io.ReadU2le()
		if err != nil {
			return err
		}
		this.QuantityCharges = append(this.QuantityCharges, tmp239)
	}
	tmp240, err := this._io.ReadBytes(int(4))
	if err != nil {
		return err
	}
	tmp240 = tmp240
	this._raw_Flags = tmp240
	_io__raw_Flags := kaitai.NewStream(bytes.NewReader(this._raw_Flags))
	tmp241 := NewCre_BodyV1_Item_ItemFlags()
	err = tmp241.Read(_io__raw_Flags, this, this._root)
	if err != nil {
		return err
	}
	this.Flags = tmp241
	return err
}

/**
 * Resource name of the ITM file.
 */
type Cre_BodyV1_Item_ItemFlags struct {
	Identified bool
	Unstealable bool
	Stolen bool
	Undroppable bool
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_Item
}
func NewCre_BodyV1_Item_ItemFlags() *Cre_BodyV1_Item_ItemFlags {
	return &Cre_BodyV1_Item_ItemFlags{
	}
}

func (this Cre_BodyV1_Item_ItemFlags) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_Item_ItemFlags) Read(io *kaitai.Stream, parent *Cre_BodyV1_Item, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp242, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Identified = tmp242 != 0
	tmp243, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Unstealable = tmp243 != 0
	tmp244, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Stolen = tmp244 != 0
	tmp245, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Undroppable = tmp245 != 0
	return err
}
type Cre_BodyV1_ItemSlot struct {
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1
}
func NewCre_BodyV1_ItemSlot() *Cre_BodyV1_ItemSlot {
	return &Cre_BodyV1_ItemSlot{
	}
}

func (this Cre_BodyV1_ItemSlot) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_ItemSlot) Read(io *kaitai.Stream, parent *Cre_BodyV1, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	return err
}
type Cre_BodyV1_KnownSpell struct {
	ResName string
	SpellLevel uint16
	SpellType Cre_SpellType
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1
}
func NewCre_BodyV1_KnownSpell() *Cre_BodyV1_KnownSpell {
	return &Cre_BodyV1_KnownSpell{
	}
}

func (this Cre_BodyV1_KnownSpell) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_KnownSpell) Read(io *kaitai.Stream, parent *Cre_BodyV1, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp246, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp246 = kaitai.BytesTerminate(tmp246, 0, false)
	this.ResName = string(tmp246)
	tmp247, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.SpellLevel = uint16(tmp247)
	tmp248, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.SpellType = Cre_SpellType(tmp248)
	return err
}

/**
 * Resource name of the SPL file
 */

/**
 * Spell level minus 1
 */
type Cre_BodyV1_MemorizedSpell struct {
	ResName string
	Flags *Cre_BodyV1_MemorizedSpell_MemorizationFlags
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1
	_raw_Flags []byte
}
func NewCre_BodyV1_MemorizedSpell() *Cre_BodyV1_MemorizedSpell {
	return &Cre_BodyV1_MemorizedSpell{
	}
}

func (this Cre_BodyV1_MemorizedSpell) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_MemorizedSpell) Read(io *kaitai.Stream, parent *Cre_BodyV1, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp249, err := this._io.ReadBytes(int(8))
	if err != nil {
		return err
	}
	tmp249 = kaitai.BytesTerminate(tmp249, 0, false)
	this.ResName = string(tmp249)
	tmp250, err := this._io.ReadBytes(int(4))
	if err != nil {
		return err
	}
	tmp250 = tmp250
	this._raw_Flags = tmp250
	_io__raw_Flags := kaitai.NewStream(bytes.NewReader(this._raw_Flags))
	tmp251 := NewCre_BodyV1_MemorizedSpell_MemorizationFlags()
	err = tmp251.Read(_io__raw_Flags, this, this._root)
	if err != nil {
		return err
	}
	this.Flags = tmp251
	return err
}

/**
 * Resource name of the SPL file.
 */
type Cre_BodyV1_MemorizedSpell_MemorizationFlags struct {
	Memorized bool
	Disabled bool
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1_MemorizedSpell
}
func NewCre_BodyV1_MemorizedSpell_MemorizationFlags() *Cre_BodyV1_MemorizedSpell_MemorizationFlags {
	return &Cre_BodyV1_MemorizedSpell_MemorizationFlags{
	}
}

func (this Cre_BodyV1_MemorizedSpell_MemorizationFlags) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_MemorizedSpell_MemorizationFlags) Read(io *kaitai.Stream, parent *Cre_BodyV1_MemorizedSpell, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp252, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Memorized = tmp252 != 0
	tmp253, err := this._io.ReadBitsIntLe(1)
	if err != nil {
		return err
	}
	this.Disabled = tmp253 != 0
	return err
}
type Cre_BodyV1_SpellMemorizationInfo struct {
	SpellLevel uint16
	NumMemorizableSpells uint16
	NumMemorizableSpellsAfterEffects uint16
	SpellType Cre_SpellType
	FirstMemorizedSpell uint32
	NumMemorizedSpell uint32
	_io *kaitai.Stream
	_root *Cre
	_parent *Cre_BodyV1
}
func NewCre_BodyV1_SpellMemorizationInfo() *Cre_BodyV1_SpellMemorizationInfo {
	return &Cre_BodyV1_SpellMemorizationInfo{
	}
}

func (this Cre_BodyV1_SpellMemorizationInfo) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_BodyV1_SpellMemorizationInfo) Read(io *kaitai.Stream, parent *Cre_BodyV1, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp254, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.SpellLevel = uint16(tmp254)
	tmp255, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.NumMemorizableSpells = uint16(tmp255)
	tmp256, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.NumMemorizableSpellsAfterEffects = uint16(tmp256)
	tmp257, err := this._io.ReadU2le()
	if err != nil {
		return err
	}
	this.SpellType = Cre_SpellType(tmp257)
	tmp258, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.FirstMemorizedSpell = uint32(tmp258)
	tmp259, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.NumMemorizedSpell = uint32(tmp259)
	return err
}
type Cre_SpellV22 struct {
	SpellIndex uint32
	AmountMemorized uint32
	AmountRemaining uint32
	_unnamed3 uint32
	_io *kaitai.Stream
	_root *Cre
	_parent kaitai.Struct
}
func NewCre_SpellV22() *Cre_SpellV22 {
	return &Cre_SpellV22{
	}
}

func (this Cre_SpellV22) IO_() *kaitai.Stream {
	return this._io
}

func (this *Cre_SpellV22) Read(io *kaitai.Stream, parent kaitai.Struct, root *Cre) (err error) {
	this._io = io
	this._parent = parent
	this._root = root

	tmp260, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.SpellIndex = uint32(tmp260)
	tmp261, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.AmountMemorized = uint32(tmp261)
	tmp262, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this.AmountRemaining = uint32(tmp262)
	tmp263, err := this._io.ReadU4le()
	if err != nil {
		return err
	}
	this._unnamed3 = tmp263
	return err
}

/**
 * Index into the relevant 2da file: listspll, listdomm, listinnt, listshap, listsong.
 */
